...
public class LogEnhance {
	...
	static class LogEnhanceMapper extends Mapper<LongWritable, Text, Text, NullWritable>{
		...
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			try {
				...
				//判断内容标签是否为空，如果为空，则只输出url到带爬清单，如果有值，则输出到增强日志
				if (content_tag == null){
					k.set(url + "\t" + "tocrawl" + "\n");
					context.write(k, v);
				}else {
	...
	public static void main(String[] args) throws Exception{
		...
		FileInputFormat.setInputPaths(job, new Path("D:\\IDEA\\HadoopDemo\\srcdata\\logen\\input"));
		// 尽管我们用的是自定义outputformat，但是它是继承制fileoutputformat
		// 在fileoutputformat中，必须输出一个_success文件，所以在此还需要设置输出path
		FileOutputFormat.setOutputPath(job, new Path("D:\\IDEA\\HadoopDemo\\srcdata\\logen\\output"));
